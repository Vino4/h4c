...
let RequestSchema = Schema({
    ...
    color: {
        type: String,
        default: RequestRandomColorGenerator,
    },
...
RequestSchema.pre('save', function(next){
    ...
    if (!this.isNew){
        Request.lastModifiedTime = Date.now();
    }
    next();
});

RequestSchema.methods.getPublicFields = function () {
    return {
        _id : this._id,
        title: this.title,
        color: this.color,
...
function RequestRandomColorGenerator(){
...
function RequestValidator(title, question) {
